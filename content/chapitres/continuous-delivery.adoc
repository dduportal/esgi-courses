...
* Un mot de passe ce n'est pas une protection suffisante :
** Expiration ? Validité ? Champ d'action (scope) ?
** ⚠️ Activez link:https://docs.docker.com/security/for-developers/2fa/[l'authentification en 2 temps (MFA) sur le DockerHub,window="_blank"]

* *Problèmes* :
** Comment faire un `docker login` dans un CI non interactif si on a le MFA ?
** Comment limiter les risques si on se fait "pwner" le mot de passe dans le CI ?

== 🎓 DockerHub Token

* *Solution* : un jeton ("token") d'authentification
** Champ d'action délimité (et idéalement réduit au maximum)
** Expiration
** Non contraint au MFA (car orienté "machine")

* Créez un link:https://docs.docker.com/docker-hub/access-tokens/["Personal Access Token" (PAT) dans le DockerHub,window="_blank"] avec :
** Les permissions "Read,Write" uniquement
** Date d'expiration dans 24h
** ⚠️ Ne partagez pas ce token, ne le mettez PAS dans GitPod

== 🎓 Secret GitHub

* *Problème* : Mais on en fait quoi de ce PAT ?
* *Solution* : On le stocke chiffré !
** GitHub permet de link:https://docs.github.com/en/actions/security-for-github-actions/security-guides/using-secrets-in-github-actions#about-secrets[stocker les secrets dans les dépôts,window="_blank"]

* Insérez le token DockerHub dans un nouveau secret dans votre dépôt GitHub :
...
** Comme avant: Lint, Build, Test de l'image (en affichant le `README`)
** SI c'est la branche `main`, alors il faut pousser l'image avec le tag `{cd_docker_tag}` sur le DockerHub
